...
function makeFullNamesList(customers){
  var customerName = [];
  man = "Mr. ";
  woman = "Ms. ";
  space = " ";
  for (i = 0; i < customers.length; i++) {
    if (customers[i].gender === "female") {
      customerName.push(woman+customers[i].first+space+customers[i].last)
    }else if(customers[i].gender === "male"){
      customerName.push(man+customers[i].first+space+customers[i].last)
    }
  }
  return customerName;
}
...
console.assert(typeof moreNamesList[0] === 'string')
